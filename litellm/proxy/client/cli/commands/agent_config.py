"""Receipt-based configure and unconfigure of a coding agent's config document.

Claude Code keeps JSON at ~/.claude/settings.json and Codex keeps TOML at ~/.codex/config.toml,
but the undo problem is the same for both: remember what every owned key held before the first
configure, fingerprint what was written, and on unconfigure put back only the keys still holding
what was written. This module owns that mechanism; the per-agent modules own the paths, the owned
keys and the merge.
"""
...
def _restore_receipt(state_path: Path, earlier: ConfigureReceipt | None) -> None:
    if earlier is None:
        ...
        return
    commit_staged_json(_stage(state_path, json.dumps(earlier.model_dump(mode="json"), indent=2)), str(state_path))
