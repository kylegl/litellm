...
def _listing_error(base_url: str, error: PiSyncError, agent: str = "Claude Code") -> str:
...
def _session(ctx: click.Context, credential: ClaudeCredential, key: str) -> _Session:
    return _Session(ctx.obj["base_url"], credential, key, is_interactive())
...
def _apply_codex(session: _Session, listed: tuple[str, ...], model: str | None) -> None:
    ...
    try:
        configure_codex_config(
            session.base_url,
            session.credential,
            lambda: (resolve_api_key_helper(session.base_url),),
            choice,
            context_window,
            config_path,
            state_path,
...
def _target_choices() -> list[Choice]:  # mutable-ok: InquirerPy takes a list
    return [
        Choice(value, name=label, enabled=value == _CLAUDE_TARGET) for value, label in _TARGETS
    ]  # mutable-ok: InquirerPy takes a list
...
def interactive_configure(
    ...
    confirm_launch: Callable[[str], bool] | None = None,
) -> None:
    ...
    if _CLAUDE_TARGET in targets:
        try:
            credential: ClaudeCredential = _claude_credential(ctx, None)
        except ClaudeSettingsError as e:
            raise click.ClickException(str(e))
        key: str = credential.token
    else:
        credential, key = _codex_credential(ctx, None)
    session: Final = _session(ctx, credential, key)
    if _CLAUDE_TARGET in targets:
        claude_listing: Final = _claude_listing(session.base_url, key)
        _apply_claude(
            session,
            claude_listing,
            pick_model(tuple(model.source_model or model.id for model in claude_listing.models)),
        )
    if _CODEX_TARGET in targets:
        codex_models: Final = _codex_listing(session.base_url, key)
        _apply_codex(session, codex_models, pick_model(_CODEX_TARGET, codex_models))
    if confirm_launch is None:
        return
...
def configure_claude(ctx: click.Context, api_key: str | None, model: str | None, launch: bool) -> None:
    ...
    try:
        ...
        raise click.ClickException(str(e))
    session: Final = _session(ctx, credential, credential.token)
...
def configure_codex(ctx: click.Context, api_key: str | None, model: str | None, launch: bool) -> None:
    ...
    try:
        credential, key = _codex_credential(ctx, api_key)
    except ClaudeSettingsError as e:
        raise click.ClickException(str(e))
    session: Final = _session(ctx, credential, key)
    _apply_codex(session, _codex_listing(session.base_url, key), model)
    if launch:
