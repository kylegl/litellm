...
# Your bearer token
token = "sk-litellm-test-master-key"
        ...
        try:
            await user_info(
                ...
                user_api_key_dict=UserAPIKeyAuth(
                    api_key="sk-litellm-test-master-key", user_id="default_user_id"
                ),
            )
        except Exception:
            pass
...
async def test_gemini_pass_through_endpoint():
    ...
    scope = {
        ...
        "query_string": b"key=sk-litellm-test-master-key",
        "headers": [
    ...
    resp = await gemini_proxy_route(
        endpoint="v1beta/models/gemini-2.5-flash:countTokens?key=sk-litellm-test-master-key",
        request=request,
        fastapi_response=Response(),
    )
