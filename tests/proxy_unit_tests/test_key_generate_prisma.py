...
def test_call_with_end_user_over_budget(prisma_client):
    ...
    async def test():
        ...
        request = Request(scope={"type": "http"})
        request._url = URL(url="/chat/completions")
        bearer_token = "Bearer sk-litellm-test-master-key"
        ...
        await proxy_db_logger._PROXY_track_cost_callback(
            kwargs={
                ...
                "litellm_params": {
                    "metadata": {
                        "user_api_key": "sk-litellm-test-master-key",
...
def test_get_bearer_token():
    ...
    api_key = "Bearer sk-litellm-test-master-key"
    result = _get_bearer_token(api_key)
    assert result == "sk-litellm-test-master-key", f"Expected 'valid_token', got '{result}'"
...
async def test_custom_api_key_header_name(prisma_client):
    ...
    request = Request(
        {
            ...
            "path": api_route.path,
            "headers": [
                (b"x-litellm-key", b"Bearer sk-litellm-test-master-key"),
            ],
        }
    )
    ...
    with pytest.raises(
        ...
        result = await user_api_key_auth(request=request, api_key="Bearer sk-litellm-test-master-key")
    e = exc_info.value
